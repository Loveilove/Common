...
//		}
		Cursor cursor = null;
        try {
            String sql = "SELECT COUNT(*) AS c FROM sqlite_master WHERE type = 'table' AND name = '"+ tableName + "' COLLATE NOCASE;";
...
//                	TABLES.put(tableName, true);
	 ...
	 * @param clazz 对象映射
	 */
	public final synchronized static <T> void createTable(SQLiteDatabase db,Class<T> clazz) {
		if(!clazz.isAnnotationPresent(Table.class)){
			...
			for(Field field:columns){
				if(field.isAnnotationPresent(Column.class)){
					...
					int columLength = column.length();
					if(columLength > 0){
						sql.append("(");
						sql.append(columLength);
						sql.append(")");
					}
...
//	 */
	/*public static <T> void createTable(SQLiteDatabase db, Class<T> clazz,boolean bool) {
		...
		sb.right_menu_delete(sb.length() - 2, sb.length() - 1);
		sb.append(")");
